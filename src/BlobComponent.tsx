...
const BlobMesh: React.FC<{ audioUrl: string }> = ({ audioUrl }) => {
  ...
  useFrame(({ clock }) => {
    if (meshRef.current?.material) {
      ...
      if (analyzer.current) {
        ...
        (meshRef.current.material as THREE.ShaderMaterial).uniforms.uFreq.value = avgFreq / 256;
      }
    }
  });
  ...
  return (
    ...
    >
      <PositionalAudio url={audioUrl} distance={7} loop ref={sound} autoplay={true} />
